...
const DEFAULT_LINE_TYPES = new Map([
  ...
  ['2', { label: '虚线-直线', type: 'linear', strokeDasharray: '6 4', value: 2 }],
  ['3', { label: '点线-直线', type: 'linear', strokeDasharray: '2 4', value: 3 }],
  ['4', { label: '点划线-直线', type: 'linear', strokeDasharray: '6 2 2 2', value: 4 }],
])
...
export const ControlMain = forwardRef<any, ControlMainProps>((props, ref) => {
  ...
  switch (type) {
    ...
    default:
      return (
        <div className={cn('flex items-center', props.controlclass, props.className0)}>
          <span className={cn('text-xs', props.controlclass)}>{props.value}</span>
        </div>
      )
  }
})
...
const ControlInput = forwardRef<HTMLInputElement, ControlMainProps>((props, ref) => {
  const {
    id,
    name,
    maxlen,
...
const ControlSelect = forwardRef<HTMLButtonElement, ControlMainProps>((props, ref) => {
  const { id, name, onChange, value, controlclass, options = [], isnum, disabled, extendcontrol, className0 } = props
    ...
    <div className={cn('flex items-center', className0)}>
      ...
      >
        <SelectTrigger
          id={id}
          ref={ref}
          className={cn('w-40 h-6 leading-6 text-xs bg-white rounded-none pl-2', controlclass)}
        >
          <SelectValue />
        </SelectTrigger>
        <SelectContent className="z-[9999]">
          {options.map((opt: { value: any; name: any }) => (
            <SelectItem key={opt.value} value={opt.value.toString()} className="text-xs pl-2">
              {opt.name}
            </SelectItem>
          ))}
        </SelectContent>
...
const ControlCheckbox = forwardRef<HTMLButtonElement, ControlMainProps>((props, ref) => {
  const { id, name, value, extendcontrol, className0, checkedClassName, onClick } = props

  return (
    <div className={cn('flex items-center', className0)}>
      <Checkbox id={id} name={name} ref={ref} checked={Boolean(value)} onClick={onClick} className={checkedClassName} />
      {extendcontrol}
    </div>
  )
})
...
const ControlRadioGroup = forwardRef<HTMLButtonElement, ControlMainProps>((props, _ref) => {
  const { onChange, value, extendcontrol, isnum, options = [], radioClassName, labelClassName } = props

  return (
    <RadioGroup value={value?.toString()} onValueChange={val => onChange?.(isnum ? Number(val) : val)}>
      {options.map((opt: { value: any; name: any; disabled?: number; extendcontrol?: any }) => (
        <label
          key={opt.value}
          className={cn(
        ...
        >
          <RadioGroupItem value={opt.value} className={radioClassName} disabled={!!opt.disabled} />
...
const ControlMenuButton = forwardRef<HTMLButtonElement, ControlMainProps>((props, ref) => {
  const {
    btnClick,
    disabled,
    menus,
  ...
  return (
    <div className={cn('flex items-center', controlclass, className0, 'pl-0')}>
      {showMenu && (
        <Menubar className="h-5 rounded-none p-0">
          <MenubarMenu>
            <MenubarTrigger ref={ref} className="p-0 rounded-none h-5" disabled={disabled}>
              <div
                className={cn(
            ...
            <MenubarContent className="z-[9999] min-w-0 rounded-none">
              {menus?.map((menu: { name: string; action: () => void }, i: number) => (
                <MenubarItem className="text-xs p-1 h-6" key={i} onClick={menu.action}>
                  {menu.name}
                </MenubarItem>
              ))}
            </MenubarContent>
      ...
      {extendcontrol && <span className="whitespace-nowrap ml-1">{extendcontrol}</span>}
    </div>
  )
})
...
const ControlColorPicker = forwardRef<HTMLButtonElement, ControlMainProps>((props, ref) => {
  const { id, name, onChange, value, controlclass, extendcontrol, colors = DEFAULT_COLORS, className0 } = props
      ...
      <Select name={name} onValueChange={onChange} value={value?.toString()}>
          ...
          <SelectValue>
            {typeof Number(value) === 'number' ? (
              <div className="flex items-center gap-2">
                <div className="w-4 h-4" style={{ background: colors[value] }} />
                {value}
              </div>
            ) : (
        ...
        <SelectContent className="z-[9999] w-24 min-w-24 max-h-40 overflow-y-auto">
          {Object.entries(colors).map(([key, color]) => (
            <SelectItem key={key} value={key} className="text-xs pl-2 flex items-center gap-2">
              <div className="flex items-center gap-2">
                <div className="w-4 h-4" style={{ background: color as React.CSSProperties['background'] }} />
                {key}
              </div>
            </SelectItem>
          ))}
...
const SvgLine = ({ pattern, w = 60 }: { pattern: string; w?: number }) => (
  <svg width={w} height="10" className="flex-shrink-0">
    <line x1="0" y1="5" x2={w} y2="5" stroke="black" strokeWidth="1" strokeDasharray={pattern} />
  </svg>
)

const ControlLineStyle = forwardRef<HTMLButtonElement, ControlMainProps>((props, ref) => {
        ...
        >
          <SelectValue>
            {typeof Number(value - 1) === 'number' ? (
              <SvgLine pattern={DEFAULT_LINE_TYPES.get(value?.toString())?.strokeDasharray ?? ''} />
            ) : (
...
const ControlLineWidth = forwardRef<HTMLButtonElement, ControlMainProps>((props, ref) => {
  const { id, name, onChange, value, controlclass, extendcontrol } = props
...
const ControlVerticalNumber = forwardRef<HTMLInputElement, ControlMainProps>((props, ref) => {
  const { id, name, onChange, value, controlclass, minlen = 0, maxlen = 100, disabled } = props
  ...
  return (
    ...
    >
      ...
      />
      <div className="flex flex-col border-l border-gray-300 -mt-[2px]" style={{ height: 'inherit' }}>
        <button
          type="button"
