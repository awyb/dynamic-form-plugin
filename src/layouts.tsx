...
export function SectionCard({
  ...
  return (
    <div className={cn('relative border border-gray-300 p-4 rounded-md w-full mt-4', className)}>
      <div className="absolute -top-3 bg-white left-2 px-2 text-xs flex items-center">
        {title}
      </div>
      {rtitle && (
        <div className={cn('absolute -top-3 bg-white left-2/3 text-xs flex items-center', rclassName)}>
          {rtitle}
        </div>
      )}
      {showBtn && (
    ...
    >
      {!isHidden && (
        <>
          ...
          {unit && (
            <div className={cn('w-6 h-6 leading-6 text-xs ml-2', unitclass)}>{unit}</div>
          )}
          {extendcontent}
          {error && <FormMessage className="text-xs text-red-500 ml-40 mt-0.5">{error}</FormMessage>}
        </>
      )}
    </FormItem>
  )
