...
export function useSchemaRenderer(ctx: RenderContext) {
  const { control, trigger, errors, precision, spaceY, showInlineErrors, onValidationError } = ctx
  ...
  const validateOne = useCallback(
    async (name: string) => {
      ...
      if (!ok) {
        const msg = typeof errors[name]?.message === 'string' ? errors[name]?.message : ''
  ...
  const renderNode = useCallback(
    (schema: FormNode): React.ReactNode => {
      ...
      if (schema.type === 'control' && schema.flds) {
        return (
          <div className={cn('flex flex-row items-center', schema.props?.className)} key={schema.key}>
            {schema.flds.map(fld => {
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItemContent
                        ...
                        noHtmlFor={fld.type === 4 || fld.type === 8 || fld.type === 0 || fld.type === 999}
                        error={showInlineErrors && fieldError ? String(fieldError.message ?? '') : undefined}
                        control={
                          <ControlMain
                            {...field}
                            {...fld}
                            ...
                            value={fld.type === 3 ? Boolean(field.value) : field.value}
                            onClick={(e: any) => {
                            ...
                            onBlur={(e: any) => {
                              ...
                              if (
                                ...
                                const formatted = parseFloat(Number(e.target.value).toPrecision(precision))
      ...
      if (schema.type === 'container') {
        return (
          <SectionCard
            ...
            {...schema.props}
          >
            {schema.children?.map(child => <React.Fragment key={child.key}>{renderNode(child)}</React.Fragment>)}
          </SectionCard>
        )
      }
      ...
      if (schema.type === 'default') {
        return (
          <div className={cn(spaceY, schema.props?.className)} key={schema.key || Math.random()} {...schema.props}>
            {schema.children?.map(child => <React.Fragment key={child.key}>{renderNode(child)}</React.Fragment>)}
          </div>
        )
      }
